...
export function renderCarouselGrid(renderedBlocks, config) {
    ...
    return `<div class="grid grid-flow-col ${rowClass} ${autoColsClass} overflow-x-auto snap-x snap-mandatory scroll-smooth gap-4 py-4 -my-4 ${pbClass} h-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">${renderedBlocks.join('')}</div>`;
}
