...
export function renderNavigationControls(config, response) {
  const baseClass = `absolute top-1/2 -translate-y-1/2 z-10 flex items-center justify-center w-8 h-8 rounded-full cursor-pointer opacity-50 hover:opacity-100 ${config.chipUI}`;
  ...
  const prevBtn = `<button class="${baseClass} left-2 ${prevClass}" type="button" title="Previous" aria-label="Previous">
            <svg class='w-5 h-5' viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><use href="#icon-caret-left"></use></svg>
          </button>`;
  const nextBtn = `<button class="${baseClass} right-2 ${nextClass}" title="Next" type="button" aria-label="Next">
            <svg class='w-5 h-5' viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><use href="#icon-caret-right"></use></svg>
          </button>`;
