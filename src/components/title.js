...
const TITLE_RENDERERS = {
    [BLOCK_QUOTE]: (title, url, hasCta, palette) => `<svg class="float-left" xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" viewBox="0 0 16 16"><use href="#icon-quote"></use></svg><blockquote class="text-left mt-2 ml-6 text-headline-sm italic">${hasCta ? title : `<a href="${url}" class="after:absolute after:inset-0 z-10" aria-label="Read more about ${title.replace(/"/g, '&quot;')}">${title}</a>`}</blockquote>`,
    [BLOCK_COVER]: (title, url, hasCta) => `<h3 class="line-clamp-2 text-display-sm font-bold mx-0 md:mx-10 opacity-50">${hasCta ? title : `<a href="${url}" class="after:absolute after:inset-0 z-10" aria-label="Read more about ${title.replace(/"/g, '&quot;')}">${title}</a>`}</h3>`,
