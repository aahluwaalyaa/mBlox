...
export function renderCTA(finalType, config, postTitle, postUrl) {
    ...
    if (finalType === BLOCK_STACK || finalType === BLOCK_CARD || finalType === BLOCK_COMMENT || finalType === BLOCK_PANCAKE || finalType === BLOCK_QUOTE) {
    ...
    const baseClasses = `js-cta-link inline-block text-label-lg font-bold no-underline px-4 py-2 hover:opacity-100 transition-opacity opacity-50 ${themeClasses} ${isSharp ? 'rounded-none' : 'rounded-full'}`;
